...
def report(
    ...
    top_k_categories: int = typer.Option(5, help="Количество выводимых top-значений для категориальных признаков"),
    title: str = typer.Option("report", help = "Заголовок в файле report.md")
) -> None:
    """
    ...
    top_cats = top_categories(df=df, top_k=top_k_categories)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть дубликаты идентификаторов: **{quality_flags['has_suspicious_id_duplicates']}**\n")
        f.write(f"- Есть категориальные признаки с большим числом унпикальных значений: **{quality_flags['has_high_cardinality_categoricals']}**\n\n")
